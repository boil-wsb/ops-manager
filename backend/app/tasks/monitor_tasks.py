...
async def check_ping(target: str, timeout: int = 10) -> tuple[bool, str | None, int | None]:
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=timeout) as client:
            ...
            if expected_content and expected_content not in response.text:
                return False, "Expected content not found in response", elapsed_ms
...
async def perform_check(monitor) -> tuple[bool, str | None, int | None]:
...
def check_monitor(self, monitor_id: int):
    ...
    from app.models.monitor import Alert, AlertSeverity, Monitor, MonitorStatus
